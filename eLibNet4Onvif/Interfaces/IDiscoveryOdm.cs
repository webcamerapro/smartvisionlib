...
namespace eLibNet4Onvif.Interfaces
{
    ...
    public interface IDiscoveryOdm
    {
        ...
        void Start(int timeOut, Action<INvtNode> discoveredAction);
        ...
        /// <exception cref="Exception">Выбрасывается, если поиск уже запущен.</exception>
        IAsyncEnumerable<INvtNode> StartAsync(int timeOut, CancellationToken cancellationToken);
        ...
        /// <exception cref="Exception">Выбрасывается, если поиск уже запущен.</exception>
        Task StartAsync(ChannelWriter<INvtNode> channelWriter, int timeOut, CancellationToken cancellationToken);
    }
}