...
namespace eLibNet4Core.Helpers
{
    ...
    public static class MathHelper
    {
        ...
        public static string BaseConvert(string value, int from, int to)
        {
            if (from == to)
                return value;
            if (from == 10 && long.TryParse(value, out var number))
                return BaseConvertTo(number, to);
            if ((from < 10 && RegexConstants.NumberRegex.IsMatch(value)) || from > 10)
                return to == 10 ? BaseConvertFrom(value, from).ToString() : BaseConvertTo(BaseConvertFrom(value, from), to);
            throw new ArgumentException("Несоответствие выбранной системе счисления");
        }
