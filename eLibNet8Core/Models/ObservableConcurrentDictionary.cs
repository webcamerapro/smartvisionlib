...
[Serializable]
public class ObservableConcurrentDictionary<TKey, TValue> : ConcurrentDictionary<TKey, TValue?>, IDictionary, IDictionary<TKey, TValue?>, IReadOnlyDictionary<TKey, TValue?>, INotifyCollectionChanged, INotifyPropertyChanged where TKey : notnull
{
    ...
    public new bool TryUpdate(TKey key, TValue? value, TValue? comparisonValue)
    {
    ...
    public new TValue? GetOrAdd(TKey key, TValue? value) => TryGetValue(key, out var existingValue) ? existingValue : TryAdd(key, value) ? value : default;
