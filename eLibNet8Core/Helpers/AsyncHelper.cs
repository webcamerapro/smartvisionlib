...
public static class AsyncHelper
{
    ...
    /// </summary>
    /// <param name="func">Функция, представляющая асинхронную задачу.</param>
    public static void RunAsyncAndReturnToCurrentThread([InstantHandle(RequireAwait = true)] Func<ValueTask> func)
    {
        var oldSyncContext = SynchronizationContext.Current;
        try
        {
    ...
    public static TResult RunAsyncAndReturnToCurrentThread<TResult>(
        [InstantHandle(RequireAwait = true)] Func<Task<TResult>> func)
    {
        var oldSyncContext = SynchronizationContext.Current;
        try
        {
    ...
    /// <returns>Результат выполнения асинхронной задачи.</returns>
    public static TResult RunAsyncAndReturnToCurrentThread<TResult>(
        [InstantHandle(RequireAwait = true)] Func<ValueTask<TResult>> func)
    {
        var oldSyncContext = SynchronizationContext.Current;
        try
        {
